TYPE=VIEW
query=select `course_backend_db`.`user_tbl`.`email` AS `email`,`course_backend_db`.`user_tbl`.`nama_depan` AS `nama_depan`,`course_backend_db`.`user_tbl`.`nama_belakang` AS `nama_belakang`,`course_backend_db`.`kota`.`nama_kota` AS `kota`,`course_backend_db`.`provinsi`.`nama_provinsi` AS `provinsi`,`course_backend_db`.`user_game_data_tbl`.`score` AS `score` from `course_backend_db`.`user_game_data_tbl` join `course_backend_db`.`user_tbl` join `course_backend_db`.`kota` join `course_backend_db`.`provinsi` where `course_backend_db`.`user_game_data_tbl`.`game_id` = 7 and `course_backend_db`.`user_game_data_tbl`.`nik` = `course_backend_db`.`user_tbl`.`nik` and `course_backend_db`.`user_tbl`.`kota_id` = `course_backend_db`.`kota`.`kota_id` and `course_backend_db`.`kota`.`provinsi_id` = `course_backend_db`.`provinsi`.`provinsi_id` and `course_backend_db`.`user_tbl`.`status` = 1 and `course_backend_db`.`user_game_data_tbl`.`status` = 1 group by `course_backend_db`.`user_tbl`.`nik` order by `course_backend_db`.`user_game_data_tbl`.`score` desc
md5=8fe091818f5f004f1def5784081249d5
updatable=0
algorithm=0
definer_user=root
